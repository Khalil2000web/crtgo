  ...
  {data.phone && (
    ...
    >
      ...
      <Tab.Group>
        ...
        {/* 🔸 Tabs Row */}
        <Tab.List className="flex gap-3 overflow-x-auto px-4 py-6 mt-10 w-[85%] flex flex-rowitems-center justify-around md:w-[60%] relative mx-auto z-50">
          {sections.map((section, i) => (
            <Tab key={i} as={Fragment}>
              {({ selected }) => (
                <button
                  className={`px-4 py-2 cursor-pointer rounded-full text-sm font-semibold border border-black whitespace-nowrap transition
                    ${selected 
                      ? "bg-black text-white" 
        ...
        <Tab.Panels className="w-full">
          ...
          {sections.map((section, i) => (
            <Tab.Panel key={i}>
              ...
              {/* Items Grid */}
              <div className="grid grid-cols-2 md:grid-cols-3 gap-6 w-full md:w-[60%] mx-auto p-4">
                  ...
                  <div key={idx} className="flex flex-col items-center gap-4">
                    ...
                    <div className="relative w-[110px] h-[110px]">
                      <Image
                        ...
                        alt={item.name[lang]}
                        fill
                        className="object-cover rounded-md pointer-events-none block"
                      />
                    </div>

                    {/* Text */}
                    <div className="flex flex-col gap-1 text-center">
                      <p className="font-semibold flex items-center text-lg justify-center gap-2">
                        {item.name[lang]}
                        {item.spicy && (
